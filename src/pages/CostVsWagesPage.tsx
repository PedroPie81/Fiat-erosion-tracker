...
const CostVsWagesPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-zinc-300 py-12 px-4 sm:px-6 font-sans">
      <Helmet>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center gap-2 mb-8 text-xs uppercase tracking-widest text-zinc-500 font-bold">
          ...
          <span>/</span>
          <span className="text-zinc-400">Cost of Living vs. Wages & Hours Worked</span>
        </div>

        {/* Hero Header */}
        <header className="bg-zinc-900/60 border border-zinc-800 rounded-3xl p-6 sm:p-10 mb-10 relative overflow-hidden">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
            <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-[#f97316]">
              <Clock className="h-4 w-4" /> Labor Purchasing Power & Real Wage Analytics
            </div>
          ...
          <p className="text-base sm:text-lg text-zinc-300 font-light leading-relaxed max-w-4xl mb-8">
            When prices are measured in rapidly depreciating fiat currency, everything appears to increase in nominal dollar/pound/euro value. When measured in <strong className="text-white font-semibold">hours of human labor</strong> required to purchase life’s essentials, we uncover the true toll of <strong className="text-[#f97316] font-semibold">fiat currency erosion</strong>.
          </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-6 border-t border-zinc-800/80">
            <div className="bg-zinc-950/70 p-4 rounded-2xl border border-zinc-800">
              <div className="text-[11px] uppercase text-zinc-500 font-bold flex items-center gap-1.5 mb-1">
                <Home className="h-3.5 w-3.5 text-amber-400" /> Median House Labor Cost
              </div>
              ...
              <div className="text-xs text-zinc-400 mt-1">
                Was {Math.round(baseHomeHours).toLocaleString()} hrs in {baseYear} ({formatDaysOrYears(targetHomeHours)})
              </div>
            </div>

            <div className="bg-zinc-950/70 p-4 rounded-2xl border border-zinc-800">
              <div className="text-[11px] uppercase text-zinc-500 font-bold flex items-center gap-1.5 mb-1">
                <DollarSign className="h-3.5 w-3.5 text-emerald-400" /> Median Wage ({currency})
              </div>
            ...
            <div className="bg-zinc-950/70 p-4 rounded-2xl border border-zinc-800">
              <div className="text-[11px] uppercase text-zinc-500 font-bold flex items-center gap-1.5 mb-1">
                <Shield className="h-3.5 w-3.5 text-[#f97316]" /> 1 oz Physical Gold (XAU)
              </div>
              ...
              <div className="text-xs text-zinc-400 mt-1">
                Was {baseGoldHours.toFixed(1)} hrs in {baseYear}
              </div>
            </div>
          </div>
        </header>
        ...
        <section className="bg-zinc-900/40 border border-zinc-800 rounded-3xl p-6 sm:p-8 mb-10">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 pb-6 border-b border-zinc-800">
            <div>
              <h2 className="text-xl font-extrabold text-white flex items-center gap-2">
                <Sliders className="h-5 w-5 text-[#f97316]" /> Interactive Historical Comparison
              </h2>
              <p className="text-xs text-zinc-400 mt-0.5">
                Select your baseline era, comparison year, and display metric:
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold uppercase tracking-wider text-zinc-400">Base Year:</span>
                <select
                  value={baseYear}
                  onChange={(e) => setBaseYear(Number(e.target.value) as SupportedYear)}
                  className="bg-zinc-950 border border-zinc-700 rounded-xl px-3 py-2 text-xs font-bold text-zinc-200 focus:outline-none focus:ring-2 focus:ring-[#f97316] cursor-pointer"
                >
              ...
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold uppercase tracking-wider text-zinc-400">Target Year:</span>
                <select
                  value={targetYear}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 items-center">
            ...
            <div className="bg-zinc-950/80 p-4 rounded-2xl border border-zinc-800/80">
              <label className="text-xs font-bold text-zinc-300 flex items-center justify-between mb-2">
                ...
                {customHourlyWage > 0 && (
                  <button 
                    ...
                    className="text-[10px] text-[#f97316] hover:underline uppercase font-bold"
                  >
                    Reset to Average ({formatMoney(targetWageData.averageHourlyWage)}/hr)
                  </button>
                )}
              </label>
              ...
              <div className="flex items-center gap-3">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full bg-zinc-900 border border-zinc-700 rounded-xl pl-8 pr-4 py-2 text-sm text-white font-mono focus:outline-none focus:ring-2 focus:ring-[#f97316]"
                  />
                </div>
                <span className="text-xs text-zinc-400 whitespace-nowrap">
                  {customHourlyWage > 0 ? `Annual: ~${formatMoney(customHourlyWage * 2000)}/yr` : 'Using national average wage'}
                </span>
              </div>
            </div>
          ...
          <div className="flex items-center gap-2 mt-6 pt-6 border-t border-zinc-800/80 overflow-x-auto pb-1 text-xs">
            <span className="text-[10px] uppercase font-bold text-zinc-500 shrink-0">Category:</span>
            ...
            <button
              ...
              className={`shrink-0 px-3 py-1.5 rounded-xl font-bold transition-all flex items-center gap-1.5 cursor-pointer ${
                categoryFilter === 'housing' 
                  ? 'bg-amber-500 text-zinc-950 shadow' 
                  : 'bg-zinc-950 text-amber-400 hover:text-white border border-zinc-800'
              }`}
            >
              <Home className="h-3.5 w-3.5" /> Housing & Rent
            </button>
            ...
            >
              <GraduationCap className="h-3.5 w-3.5" /> Education & Health
            </button>
            ...
            >
              <Car className="h-3.5 w-3.5" /> Cars & Gasoline
            </button>
          ...
          <div className="flex justify-between items-end mb-6">
            <div>
              <h2 className="text-2xl font-extrabold text-white">
                Historical Cost vs. Wage Matrix ({baseYear} vs. {targetYear})
              </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {filteredItems.map((item) => {
              ...
              return (
                <div 
                  key={item.id}
                  className="bg-zinc-950 border border-zinc-800/80 hover:border-zinc-700/80 rounded-2xl p-6 transition-all flex flex-col justify-between shadow-lg group"
                >
                  <div>
                    {/* Header */}
                    <div className="flex items-start justify-between gap-3 mb-3">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          <h3 className="font-extrabold text-base text-white group-hover:text-[#f97316] transition-colors">
                            {item.name}
                          </h3>
                          <span className="text-[11px] text-zinc-500 font-medium">
                            {item.unit} • {item.categoryLabel}
                          </span>
                        </div>
                      </div>

                      {hasBasePrice && (
                        <span className={`px-2.5 py-1 rounded-full text-[11px] font-extrabold uppercase tracking-wide border shrink-0 ${
                    ...
                    {displayMode === 'hours' && (
                      <div className="grid grid-cols-2 gap-4 p-4 rounded-xl bg-zinc-900/60 border border-zinc-800/70 mb-4">
                        <div>
                          <div className="text-[10px] uppercase font-bold text-zinc-500">{baseYear} Labor Required</div>
                          <div className="text-xl font-extrabold font-mono text-zinc-300 mt-1">
                            {hasBasePrice ? formatHours(baseHours) : 'N/A (Pre-launch)'}
                          </div>
                          {hasBasePrice && (
                        ...
                        <div className="border-l border-zinc-800 pl-4">
                          <div className="text-[10px] uppercase font-bold text-zinc-500">{targetYear} Labor Required</div>
                          <div className={`text-xl font-extrabold font-mono mt-1 ${isMoreExpensiveInLabor ? 'text-rose-400' : 'text-emerald-400'}`}>
                    ...
                    {displayMode === 'nominal' && (
                      <div className="grid grid-cols-2 gap-4 p-4 rounded-xl bg-zinc-900/60 border border-zinc-800/70 mb-4">
                        <div>
                          <div className="text-[10px] uppercase font-bold text-zinc-500">{baseYear} Nominal Price</div>
                          <div className="text-xl font-extrabold font-mono text-zinc-300 mt-1">
                        ...
                        <div className="border-l border-zinc-800 pl-4">
                          <div className="text-[10px] uppercase font-bold text-zinc-500">{targetYear} Nominal Price</div>
                          <div className="text-xl font-extrabold font-mono text-[#f97316] mt-1">
                    ...
                    {displayMode === 'wage_multiplier' && (
                      <div className="grid grid-cols-2 gap-4 p-4 rounded-xl bg-zinc-900/60 border border-zinc-800/70 mb-4">
                        <div>
                          <div className="text-[10px] uppercase font-bold text-zinc-500">{baseYear} % of Median Wage</div>
                          <div className="text-xl font-extrabold font-mono text-zinc-300 mt-1">
                        ...
                        <div className="border-l border-zinc-800 pl-4">
                          <div className="text-[10px] uppercase font-bold text-zinc-500">{targetYear} % of Median Wage</div>
                  ...
                  {/* Footnote / Context */}
                  {item.notes && (
                    <div className="pt-3 border-t border-zinc-900 text-[11px] text-zinc-400 flex items-start gap-1.5 italic">
        ...
        <section className="bg-zinc-900/40 border border-zinc-800 rounded-3xl p-6 sm:p-8 mb-14 overflow-hidden">
          ...
          <p className="text-zinc-400 text-sm mb-6">
            Examine how wages and core life essentials have transformed across every major decade in {dataset.name}:
          </p>
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="border-b border-zinc-800 bg-zinc-950/80 text-zinc-400 font-bold uppercase tracking-wider">
                  <th className="py-3.5 px-4">Item / Metric</th>
                  {YEARS_LIST.map(year => (
                    <th key={year} className="py-3.5 px-4 font-mono text-right">
                      {year}
                    </th>
                  ))}
                </tr>
              ...
              <tbody className="divide-y divide-zinc-800/60 font-mono">
                ...
                <tr className="bg-orange-950/20 text-[#f97316] font-bold">
                  <td className="py-3 px-4 font-sans flex items-center gap-1.5">
                    <DollarSign className="h-3.5 w-3.5" /> Median Annual Wage
                  </td>
                  {YEARS_LIST.map(year => (
                ...
                <tr className="bg-orange-950/10 text-orange-300 font-bold">
                  <td className="py-3 px-4 font-sans flex items-center gap-1.5">
                    <Clock className="h-3.5 w-3.5" /> Average Hourly Wage
                  </td>
                  {YEARS_LIST.map(year => (
                    <td key={year} className="py-3 px-4 text-right">
                ...
                {dataset.items.map((item) => (
                  <tr key={item.id} className="hover:bg-zinc-900/60 transition-colors">
                    <td className="py-3 px-4 font-sans text-zinc-200 font-medium flex items-center gap-2">
                      {getCategoryIcon(item.category)}
                    ...
                    {YEARS_LIST.map(year => {
                      ...
                      if (!price || price === 0) {
                        return <td key={year} className="py-3 px-4 text-right text-zinc-600">—</td>;
                      }
                      const hours = price / dataset.wages[year].averageHourlyWage;
                      return (
                        <td key={year} className="py-3 px-4 text-right text-zinc-300">
          ...
          <div className="mt-4 pt-4 border-t border-zinc-800/80 text-[11px] text-zinc-500 flex flex-wrap justify-between gap-2">
        ...
        <section className="bg-gradient-to-br from-amber-950/20 via-zinc-900 to-zinc-950 border border-amber-900/40 rounded-3xl p-6 sm:p-10 mb-14">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 my-8 text-xs">
            <div className="bg-zinc-950/80 border border-zinc-800 p-5 rounded-2xl">
              <h3 className="font-bold text-white mb-2 text-sm text-[#f97316]">
            ...
            <div className="bg-zinc-950/80 border border-zinc-800 p-5 rounded-2xl">
              <h3 className="font-bold text-white mb-2 text-sm text-amber-400">
            ...
            <div className="bg-zinc-950/80 border border-zinc-800 p-5 rounded-2xl">
              <h3 className="font-bold text-white mb-2 text-sm text-emerald-400">
                3. Asset Inflation vs. CPI Distortion
              </h3>
              <p className="text-zinc-400 leading-relaxed">
                Official Consumer Price Index (CPI) metrics use geometric weighting, hedonic adjustments, and owner's equivalent rent to hide real asset price inflation, masking the explosive cost of housing, healthcare, and higher education.
              </p>
            </div>
          </div>
        ...
        <section className="bg-zinc-900/30 border border-zinc-800 rounded-3xl p-6 sm:p-10 mb-16">
          <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-amber-500 mb-2">
